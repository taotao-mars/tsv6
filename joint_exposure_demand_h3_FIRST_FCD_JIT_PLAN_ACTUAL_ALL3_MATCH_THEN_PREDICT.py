...
Oracle eval-promotion feature-importance experiment:
  - uses all three realized eval-snapshot promotion features for H1-H3;
  - does not read or apply the origin-time planned-deals snapshot;
  - measures permutation importance for ind_promotion,
    promotion_pricing_amount, and pricing_type;
  - this is an oracle upper-bound experiment because realized future promotion
    status is unavailable at the forecast origin.
"""
...
def _resolve_run_asin_cohort(asin_cohort="chris", jit_asin_path=None):
    """Return chris, jit, union, or Chris/JIT intersection cohort."""
    ...
    if mode not in {"jit", "union", "intersection"}:
        raise ValueError(
            "asin_cohort must be one of: 'chris', 'jit', 'union', "
            "'intersection'."
        )
    ...
    if not CHRIS_ASIN_SET:
        raise FileNotFoundError(
            f"{mode} cohort mode requested, but {CHRIS_DF_PATH} was not found."
        )
    if mode == "intersection":
        return set(CHRIS_ASIN_SET) & jit_set, "chris_jit_intersection", resolved_path
...
PROMO_IMPORTANCE_CONTEXT_COLS = [
    "stock_extra__ind_promotion",
    "stock_extra__promotion_pricing_amount",
    "stock_extra__pricing_type",
]


def _safe_wape_np(actual, predicted):
    ...
    if not valid.any() or denom <= 0:
        return np.nan
    return float(np.abs(predicted[valid] - actual[valid]).sum() / denom)


def _evaluate_with_optional_context_permutation(
    model,
    va_ld,
    context_col_idx=None,
    M=100,
    seed=42,
):
    """Evaluate validation WAPE after optionally permuting one future feature."""
    collected = {
        "oos": [],
        "actual_demand": [], "p50": [], "p70": [], "p90": [],
        "actual_total": [], "pred_total": [],
        "actual_buybox": [], "pred_buybox": [],
        "actual_instock": [], "pred_instock": [],
    }
    model = model.to(DEVICE)
    model.eval()

    with torch.no_grad():
        for batch_idx, original_batch in enumerate(va_ld):
            b = _move_batch_to_device(original_batch, DEVICE)
            fc = b["future_context"].clone()

            if context_col_idx is not None:
                values = fc[:, :, context_col_idx].reshape(-1)
                rng = np.random.default_rng(seed + 10007 * (batch_idx + 1))
                perm_np = rng.permutation(values.numel())
                perm = torch.as_tensor(perm_np, dtype=torch.long, device=values.device)
                fc[:, :, context_col_idx] = values[perm].reshape_as(
                    fc[:, :, context_col_idx]
                )

            # Keep NB sampling identical between baseline and every permutation.
            ...
            collected["actual_demand"].append(b["y"].detach().cpu().numpy().ravel())
            collected["oos"].append(b["oos"].detach().cpu().numpy().ravel())
            collected["p50"].append(p50.detach().cpu().numpy().ravel())
            collected["p70"].append(p70.detach().cpu().numpy().ravel())
            collected["p90"].append(p90.detach().cpu().numpy().ravel())
            collected["actual_total"].append(
                b["future_total_dph"].detach().cpu().numpy().ravel()
            )
            collected["pred_total"].append(
                exp_hat[:, :, 0].detach().cpu().numpy().ravel()
            )
            collected["actual_buybox"].append(
                b["future_buy_box_dph"].detach().cpu().numpy().ravel()
            )
            collected["pred_buybox"].append(
                exp_hat[:, :, 1].detach().cpu().numpy().ravel()
            )
            collected["actual_instock"].append(
                b["future_instock"].detach().cpu().numpy().ravel()
            )
            collected["pred_instock"].append(
                exp_hat[:, :, 2].detach().cpu().numpy().ravel()
            )

    values = {k: np.concatenate(v) for k, v in collected.items()}
    keep = np.isfinite(values["oos"]) & (values["oos"] == 0)
    values = {k: v[keep] for k, v in values.items() if k != "oos"}
    return {
        "demand_p50_wape": _safe_wape_np(values["actual_demand"], values["p50"]),
        "demand_p70_wape": _safe_wape_np(values["actual_demand"], values["p70"]),
        "demand_p90_wape": _safe_wape_np(values["actual_demand"], values["p90"]),
        "total_dph_wape": _safe_wape_np(values["actual_total"], values["pred_total"]),
        "buy_box_dph_wape": _safe_wape_np(values["actual_buybox"], values["pred_buybox"]),
        "in_stock_dph_wape": _safe_wape_np(values["actual_instock"], values["pred_instock"]),
    }


def compute_promo_permutation_importance(
    model,
    va_ld,
    context_cols,
    M=100,
    ...
    """Permutation importance of the three realized eval promotion features."""
    missing = [c for c in PROMO_IMPORTANCE_CONTEXT_COLS if c not in context_cols]
    if missing:
        raise RuntimeError(
            "Promo importance features are absent from future_context: "
            + ", ".join(missing)
        )

    baseline = _evaluate_with_optional_context_permutation(
        model=model, va_ld=va_ld, context_col_idx=None, M=M, seed=seed
    )
    rows = []
    for encoded_col in PROMO_IMPORTANCE_CONTEXT_COLS:
        permuted = _evaluate_with_optional_context_permutation(
            model=model,
            va_ld=va_ld,
            context_col_idx=context_cols.index(encoded_col),
            M=M,
            seed=seed,
        )
        feature = encoded_col.replace("stock_extra__", "")
        row = {"feature": feature}
        for metric, baseline_value in baseline.items():
            permuted_value = permuted[metric]
            row[f"baseline_{metric}"] = baseline_value
            row[f"permuted_{metric}"] = permuted_value
            row[f"importance_delta_{metric}"] = permuted_value - baseline_value
            row[f"importance_pct_{metric}"] = (
                100.0 * (permuted_value - baseline_value) / baseline_value
                if np.isfinite(baseline_value) and baseline_value > 0 else np.nan
            )
        rows.append(row)

    importance = pd.DataFrame(rows).sort_values(
        "importance_delta_demand_p50_wape", ascending=False
    ).reset_index(drop=True)
    print("\n" + "=" * 110, flush=True)
    print("REALIZED EVAL PROMOTION FEATURE PERMUTATION IMPORTANCE", flush=True)
    print("Positive WAPE delta means the feature is useful; larger is more important.", flush=True)
    print("=" * 110, flush=True)
    display_cols = [
        "feature",
        "importance_pct_demand_p50_wape",
        "importance_pct_demand_p70_wape",
        "importance_pct_total_dph_wape",
        "importance_pct_buy_box_dph_wape",
        "importance_pct_in_stock_dph_wape",
    ]
    print(importance[display_cols].round(4).to_string(index=False), flush=True)
    return importance
...
def run_joint_exposure_demand_h3_end2end(
    ...
    promo_feature_importance = compute_promo_permutation_importance(
        model=model,
        va_ld=va_ld,
    ...
    importance_path = output_csv.parent / (
        f"promo_feature_importance_{output_csv.stem}.csv"
    )
    promo_feature_importance.to_csv(importance_path, index=False)
    print(f"Saved promotion feature importance: {importance_path}", flush=True)
    ...
    result = {
        ...
        "va_ld": va_ld,
        "context_cols": context_cols,
        "promo_feature_importance": promo_feature_importance,
        "promo_feature_importance_path": str(importance_path),
...
def _planned_deals_key_for_cut(data_cut, deals_prefix=ROLLING_DEALS_PREFIX):
    """Return the origin-time ASIN deals snapshot key for one rolling cut."""
    cut_token = pd.Timestamp(data_cut).strftime("%Y%m%d")
    return f"{str(deals_prefix).rstrip('/')}/asin_deals_{cut_token}.csv000"
...
def _normalize_pricing_type_for_match(value):
...
def _find_all3_full_match_asins(
    eval_raw,
    deals_raw,
    selected_asins,
    target_weeks,
    ...
    """Find ASINs whose planned/actual three promo fields match in H1-H3."""
    fields = ["ind_promotion", "promotion_pricing_amount", "pricing_type"]
    required_eval = {"asin", "order_week", *fields}
    required_deals = {
        "asin", "asin_promo_start_week", "asin_promo_end_week",
        "promotion_pricing_amount", "pricing_type",
    }
    if required_eval - set(eval_raw.columns):
        raise KeyError(
            "Eval snapshot missing columns: "
            + ", ".join(sorted(required_eval - set(eval_raw.columns)))
        )
    if required_deals - set(deals_raw.columns):
        raise KeyError(
            "Deals snapshot missing columns: "
            + ", ".join(sorted(required_deals - set(deals_raw.columns)))
        )

    weeks = [pd.Timestamp(w).normalize() for w in target_weeks]
    horizon_map = {w: f"H{i+1}" for i, w in enumerate(weeks)}
    asins = sorted(set(str(a).strip() for a in selected_asins))
    grid = pd.MultiIndex.from_product(
        [asins, weeks], names=["asin", "order_week"]
    ).to_frame(index=False)
    grid["forecast_horizon"] = grid["order_week"].map(horizon_map)

    deals = deals_raw.copy()
    deals["asin"] = deals["asin"].astype(str).str.strip()
    ...
    deals = deals[
        deals["asin"].isin(asins)
        & deals["asin_promo_start_week"].notna()
        & deals["asin_promo_end_week"].notna()
        & (deals["asin_promo_start_week"] <= max(weeks))
        & (deals["asin_promo_end_week"] >= min(weeks))
    ].copy()
    ...
    candidates = grid.merge(deals[deal_cols], on="asin", how="left")
    active = (
    ...
    matches = candidates.loc[active].copy()
    counts = (
        matches.groupby(["asin", "order_week"], as_index=False)
        .size().rename(columns={"size": "matching_deal_count"})
    )
    if matches.empty:
        chosen = pd.DataFrame(columns=deal_cols + ["order_week"])
    else:
        matches["_amount_sort"] = matches["promotion_pricing_amount"].fillna(-np.inf)
        chosen = (
            matches.sort_values(
                ["asin", "order_week", "asin_promo_start_week",
                 "asin_promo_end_week", "_amount_sort"],
    ...
    plan = (
        grid.merge(
            chosen[["asin", "order_week", "asin_promo_start_week",
                    "asin_promo_end_week", "promotion_pricing_amount",
                    "pricing_type"]],
            on=["asin", "order_week"], how="left", validate="one_to_one",
        )
        .merge(counts, on=["asin", "order_week"], how="left", validate="one_to_one")
    )
    plan["matching_deal_count"] = plan["matching_deal_count"].fillna(0).astype(int)
    plan["planned_ind_promotion"] = plan["matching_deal_count"].gt(0).astype(int)
    plan["planned_promotion_pricing_amount"] = pd.to_numeric(
        plan["promotion_pricing_amount"], errors="coerce"
    ).fillna(0.0)
    plan["planned_pricing_type"] = np.where(
        plan["planned_ind_promotion"].eq(1), plan["pricing_type"], "NO_DEAL"
    )

    actual = eval_raw[["asin", "order_week", *fields]].copy()
    actual["asin"] = actual["asin"].astype(str).str.strip()
    actual["order_week"] = pd.to_datetime(
        actual["order_week"], errors="coerce"
    ).dt.normalize()
    actual = actual[actual["asin"].isin(asins) & actual["order_week"].isin(weeks)]
    actual["_ind_sort"] = pd.to_numeric(actual["ind_promotion"], errors="coerce").fillna(0)
    actual["_amount_sort"] = pd.to_numeric(
        actual["promotion_pricing_amount"], errors="coerce"
    ).fillna(-np.inf)
    actual = (
        actual.sort_values(
            ["asin", "order_week", "_ind_sort", "_amount_sort"],
            ascending=[True, True, False, False], kind="mergesort",
        )
        .drop_duplicates(["asin", "order_week"], keep="first")
        .drop(columns=["_ind_sort", "_amount_sort"])
        .rename(columns={c: f"actual_{c}" for c in fields})
    )

    detail = plan.merge(actual, on=["asin", "order_week"], how="left", validate="one_to_one")
    detail["missing_actual"] = detail["actual_ind_promotion"].isna()
    actual_ind = pd.to_numeric(detail["actual_ind_promotion"], errors="coerce")
    actual_amount = pd.to_numeric(
        detail["actual_promotion_pricing_amount"], errors="coerce"
    )
    detail["ind_match"] = (
        detail["planned_ind_promotion"].eq(actual_ind) & ~detail["missing_actual"]
    )
    detail["amount_match"] = (
        np.isclose(
            detail["planned_promotion_pricing_amount"].fillna(0),
            actual_amount.fillna(0), atol=amount_atol, rtol=amount_rtol,
        ) & ~detail["missing_actual"]
    )
    detail["pricing_type_match"] = (
        detail["planned_pricing_type"].map(_normalize_pricing_type_for_match)
        == detail["actual_pricing_type"].map(_normalize_pricing_type_for_match)
    ) & ~detail["missing_actual"]
    detail["strict_all3_week_match"] = (
        detail["ind_match"] & detail["amount_match"] & detail["pricing_type_match"]
    )
    detail["data_cut"] = pd.Timestamp(data_cut).normalize()

    asin_summary = (
        detail.groupby("asin", as_index=False)
        .agg(
            weeks_found=("forecast_horizon", "nunique"),
            any_missing_actual=("missing_actual", "any"),
            all_h1_h2_h3_ind_match=("ind_match", "all"),
            all_h1_h2_h3_amount_match=("amount_match", "all"),
            all_h1_h2_h3_pricing_type_match=("pricing_type_match", "all"),
            all_h1_h2_h3_all3_match=("strict_all3_week_match", "all"),
        )
    )
    full_match_mask = (
        asin_summary["weeks_found"].eq(3)
        & ~asin_summary["any_missing_actual"]
        & asin_summary["all_h1_h2_h3_all3_match"]
    )
    matched_asins = set(asin_summary.loc[full_match_mask, "asin"])
    print("\n" + "=" * 100, flush=True)
    print("CHRIS ∩ JIT → PLAN/ACTUAL ALL-3-FEATURE FULL MATCH", flush=True)
    print("=" * 100, flush=True)
    print(f"Input joint ASINs: {len(asins):,}", flush=True)
    print(f"All H1/H2/H3, all 3 fields matched ASINs: {len(matched_asins):,}", flush=True)
    print(f"Retention rate: {len(matched_asins)/len(asins):.2%}" if asins else "Retention rate: n/a", flush=True)
    return matched_asins, detail, asin_summary


def _apply_origin_planned_deals_to_h3(
    data_raw1,
    deals_raw,
    target_weeks,
    data_cut,
    ...
    if missing_eval:
        raise KeyError(
            "Evaluation snapshot is missing planned-patch columns: "
            + ", ".join(missing_eval)
        )
    if missing_deals:
        raise KeyError(
            "ASIN deals snapshot is missing required columns: "
            + ", ".join(missing_deals)
        )

    out = data_raw1.copy().reset_index(drop=True)
    ...
    if len(set(normalized_target_weeks)) != 3:
        raise ValueError(
            "Planned-deals H3 patch requires exactly three distinct target weeks."
        )

    target_mask = out["order_week"].isin(normalized_target_weeks)
    if not target_mask.any():
        raise RuntimeError(
            "No H1-H3 evaluation rows matched the planned-deals target weeks."
        )

    target_keys = (
        out.loc[target_mask, ["asin", "order_week"]]
        .drop_duplicates()
        .reset_index(drop=True)
    )
    horizon_map = {
        week: f"H{i + 1}" for i, week in enumerate(normalized_target_weeks)
    }
    target_keys["forecast_horizon"] = target_keys["order_week"].map(
        horizon_map
    )
    ...
    deals = deals[
        ...
        & deals["asin_promo_start_week"].notna()
        & deals["asin_promo_end_week"].notna()
        & (deals["asin_promo_start_week"] <= max(normalized_target_weeks))
        & (deals["asin_promo_end_week"] >= min(normalized_target_weeks))
    ].copy()
    ...
    match_counts = (
        matches.groupby(["asin", "order_week"], as_index=False)
        .size()
        .rename(columns={"size": "matching_deal_count"})
    )
    ...
    actual = actual.rename(
        columns={c: f"actual_eval_{c}" for c in PLANNED_PROMO_COLS}
    )
    diagnostic = actual.merge(
        plan[
            [
                "asin",
                "order_week",
    ...
    diagnostic["data_cut"] = pd.Timestamp(data_cut).normalize()
    ...
    for horizon_label, sub in [
        ("ALL", diagnostic),
        *[
            (label, diagnostic[diagnostic["forecast_horizon"] == label])
    ...
    if len(out) != len(data_raw1):
        raise AssertionError("Planned-deals patch changed evaluation row count.")
    if out.loc[target_mask, PLANNED_PROMO_COLS].isna().any().any():
        raise AssertionError(
            "Planned-deals patch left a null H1-H3 promotion input."
        )

    print("\n" + "=" * 104, flush=True)
    print(
        "ORIGIN-PLANNED DEALS VS EVAL-SNAPSHOT VALUES | "
        f"DATA CUT={pd.Timestamp(data_cut).date()}",
        flush=True,
    )
    print("=" * 104, flush=True)
    print(summary.round(6).to_string(index=False), flush=True)
    ...
    if mismatch_preview.empty:
        ...
        print(mismatch_preview.head(20).to_string(index=False), flush=True)

    return out, diagnostic, summary
...
def run_joint_h3_s3_rolling_scot_p50_p70(
    ...
    asin_cohort="chris",
    jit_asin_path=None,
    filter_plan_actual_all3_full_match=False,
):
    """
    ...
    if use_origin_planned_deals and resume_existing:
        raise ValueError(
    ...
    print("Filter plan/actual all 3 fields across H1-H3:", filter_plan_actual_all3_full_match)
    ...
    all_planned_deals_detail = []
    all_planned_deals_summary = []
    ...
    for run_index, row in pairs.iterrows():
        ...
        try:
            planned_deals_key_for_cut = ""
            planned_deals_summary_cut = pd.DataFrame()
            ...
            if filter_plan_actual_all3_full_match:
                match_deals_key = _planned_deals_key_for_cut(
                    data_cut=data_cut,
                    deals_prefix=planned_deals_prefix,
                )
                print(
                    f"[FULL-MATCH-FILTER] reading {match_deals_key}",
                    flush=True,
                )
                match_deals_raw = _read_s3_csv(
                    bucket, match_deals_key, s3_client
                )
                (
                    full_match_asins,
                    full_match_detail,
                    full_match_asin_summary,
                ) = _find_all3_full_match_asins(
                    eval_raw=eval_raw,
                    deals_raw=match_deals_raw,
                    selected_asins=rolling_asins,
                    target_weeks=[expected_h1, expected_h2, expected_h3],
                    data_cut=data_cut,
                )
                full_match_detail.to_csv(
                    output_root / f"plan_actual_all3_detail_cut_{cut_token}.csv",
                    index=False,
                )
                full_match_asin_summary.to_csv(
                    output_root / f"plan_actual_all3_asin_summary_cut_{cut_token}.csv",
                    index=False,
                )
                rolling_asins = sorted(set(rolling_asins) & full_match_asins)
                if not rolling_asins:
                    raise RuntimeError(
                        "No ASIN remains after Chris∩JIT and plan/actual "
                        "H1-H3 all-three-feature full-match filtering."
                    )
                data_raw1 = eval_raw[
                    eval_raw["asin"].isin(rolling_asins)
                ].copy()
                scot_df = scot_df[
                    scot_df["asin"].isin(rolling_asins)
                ].copy()
                print(
                    f"[FULL-MATCH-FILTER] final prediction cohort="
                    f"{len(rolling_asins):,}",
                    flush=True,
                )

            if use_origin_planned_deals:
                planned_deals_key_for_cut = _planned_deals_key_for_cut(
                    data_cut=data_cut,
                    deals_prefix=planned_deals_prefix,
                )
                print(
                    f"[PLANNED-DEALS] reading origin snapshot | "
                    f"data_cut={data_cut.date()} | "
                    f"key={planned_deals_key_for_cut}",
                    flush=True,
                )
                deals_raw = _read_s3_csv(
                    bucket,
                    planned_deals_key_for_cut,
                    s3_client,
                )
                (
                    data_raw1,
                    planned_deals_detail_cut,
                    planned_deals_summary_cut,
                ) = _apply_origin_planned_deals_to_h3(
                    data_raw1=data_raw1,
                    deals_raw=deals_raw,
                    target_weeks=[expected_h1, expected_h2, expected_h3],
                    data_cut=data_cut,
                )
                planned_deals_detail_cut["planned_deals_s3_key"] = (
                    planned_deals_key_for_cut
                )
                planned_deals_summary_cut["planned_deals_s3_key"] = (
                    planned_deals_key_for_cut
                )
                planned_detail_path = output_root / (
                    f"planned_vs_eval_promotion_cut_{cut_token}.csv"
                )
                planned_summary_path = output_root / (
                    f"planned_vs_eval_promotion_summary_cut_{cut_token}.csv"
                )
                planned_deals_detail_cut.to_csv(
                    planned_detail_path,
                    index=False,
                )
                planned_deals_summary_cut.to_csv(
                    planned_summary_path,
                    index=False,
                )
                print(
                    f"[PLANNED-DEALS] saved detail diagnostic: "
                    f"{planned_detail_path}",
                    flush=True,
                )
                print(
                    f"[PLANNED-DEALS] saved summary diagnostic: "
                    f"{planned_summary_path}",
                    flush=True,
                )
                all_planned_deals_detail.append(
                    planned_deals_detail_cut.copy()
                )
                all_planned_deals_summary.append(
                    planned_deals_summary_cut.copy()
                )
            ...
            summary = {
                ...
                "planned_deals_s3_key": planned_deals_key_for_cut,
            ...
            if not planned_deals_summary_cut.empty:
                promo_all = planned_deals_summary_cut[
                    planned_deals_summary_cut["forecast_horizon"] == "ALL"
                ].iloc[0]
                summary.update({
                    "planned_deal_rate": promo_all["planned_deal_rate"],
                    "actual_eval_deal_rate": promo_all[
                        "actual_eval_deal_rate"
                    ],
    ...
    if all_planned_deals_detail:
        planned_deals_detail_full = pd.concat(
            all_planned_deals_detail,
            ignore_index=True,
        )
    else:
        planned_deals_detail_full = pd.DataFrame()

    if all_planned_deals_summary:
        planned_deals_summary_full = pd.concat(
            all_planned_deals_summary,
            ignore_index=True,
        )
    else:
        planned_deals_summary_full = pd.DataFrame()
    ...
    return {
        ...
        "planned_vs_eval_promotion_detail": planned_deals_detail_full,
        "planned_vs_eval_promotion_summary": planned_deals_summary_full,
...
# ============================================================================
# USAGE 4: CHRIS ∩ JIT → ALL-3 FULL MATCH → TRAIN/PREDICT
# ============================================================================
# First intersect Chris and JIT cohorts, then retain only ASINs whose planned
# and actual ind_promotion, promotion_pricing_amount, and pricing_type all
# match in every one of H1/H2/H3. Train and predict only on that final cohort.
#   data_cut = 2025-10-04
#   FCD      = 2025-10-05
#   H1-H3    = 2025-10-05, 2025-10-12, 2025-10-19
#
# The 2025-10-04 deals table is read only to construct the full-match filter;
# planned values are never written into eval data. The 2025-10-25 snapshot
# supplies actual demand/DPH/OOS plus the three realized promotion features.
# After training, each feature is permuted separately to measure demand and
# exposure WAPE degradation.
#
# Run this cell/call after loading the file. It uses Chris ∩ JIT before the
# origin/eval/SCOT and all-three-feature full-match filters; no random 5,000
# ASIN sampling is applied.
#
...
first_fcd_chris_jit_all3_match_predict = run_joint_h3_s3_rolling_scot_p50_p70(
    n_asins=None,
    seed=42,
    max_snapshots=1,
    select_latest=False,
    ...
    lambda_exposure=0.50,
    detach_exposure_for_demand=False,
    output_root="joint_h3_first_fcd_chris_jit_all3_full_match_predict",
    resume_existing=False,
    continue_on_error=False,
    use_origin_planned_deals=False,
    asin_cohort="intersection",
    jit_asin_path=JIT_ASIN_CSV_PATH,
    filter_plan_actual_all3_full_match=True,
)
