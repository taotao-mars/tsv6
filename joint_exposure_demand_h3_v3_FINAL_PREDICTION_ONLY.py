...
def diagnose_encoder(model, va_ld):
    """
    Evaluate the quality of the encoder representation h_t:
    1. Ability of h_t to separate active and inactive samples (AUC).
    2. Predictive strength of h_t for positive-demand magnitude (R²).
    3. Calibration of mu_base against observed demand.
    """
    print("\n" + "="*60)
    ...
    # 1. Occurrence classification quality
    ...
    if has_active.sum() > 10 and (~has_active).sum() > 10:
        try:
            ...
            if auc < 0.6:
                print("  <- Weak: the encoder does not separate occurrence states well.")
            elif auc < 0.75:
                print("  <- Moderate: the representation has room for improvement.")
            else:
                print("  <- Strong: the encoder separates occurrence states well.")
        except Exception as e:
            print(f"AUC calculation failed: {e}")

    # 2. Positive-demand magnitude predictability
    ...
    if len(h_active) > 20:
        try:
            ...
            if r2 < 0.1:
                print("  <- Weak: the encoder has little predictive power for magnitude.")
            elif r2 < 0.3:
                print("  <- Moderate: the representation has room for improvement.")
            else:
                print("  <- Strong: the encoder captures demand magnitude information.")
        except Exception as e:
            print(f"R-squared calculation failed: {e}")

    # 3. Base mean calibration against observed demand
    ...
    if active_weeks_mask.sum() > 0:
        ...
        if mu_mean / max(true_mean, 1e-8) < 0.3:
            print("  <- mu_base is severely underestimated; inspect magnitude learning.")
        elif mu_mean / max(true_mean, 1e-8) < 0.7:
            print("  <- mu_base is somewhat low and may need better calibration.")
        else:
            print("  <- mu_base is reasonably calibrated.")

    # 4. Latent-variable scale
    z_means, z_stds = [], []
    with torch.no_grad():
        for b in va_ld:
    ...
    if z_stds.mean() > 3.0:
        print("  <- z_std is too large; posterior spread may destabilize joint prediction.")
    elif z_stds.mean() < 0.1:
        print("  <- z_std is too small; the latent variable carries little uncertainty.")
    else:
        print("  <- z_std is within a reasonable range.")
