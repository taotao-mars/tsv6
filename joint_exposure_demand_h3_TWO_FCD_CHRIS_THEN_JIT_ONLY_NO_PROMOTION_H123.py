...
def select_complete_h123_no_promotion_asins(
    eval_raw,
    expected_weeks,
):
    """Return ASINs whose raw eval ind_promotion is 0 in H1, H2, and H3.

    All three target weeks must be present.  With duplicate ASIN-week rows,
    max is conservative: if any duplicate reports a promotion, that week is
    promotional and the ASIN is excluded.
    """
    required = {"asin", "order_week", "ind_promotion"}
    missing = sorted(required - set(eval_raw.columns))
    if missing:
        raise KeyError(
            "Cannot select the H1-H3 no-promotion model cohort; eval snapshot "
            "is missing: " + ", ".join(missing)
        )

    target_weeks = [pd.Timestamp(w).normalize() for w in expected_weeks]
    if len(target_weeks) != 3 or len(set(target_weeks)) != 3:
        raise ValueError(
            "expected_weeks must contain three distinct H1/H2/H3 weeks."
        )

    df = eval_raw[["asin", "order_week", "ind_promotion"]].copy()
    df["asin"] = df["asin"].astype(str).str.strip()
    df["order_week"] = pd.to_datetime(
        df["order_week"], errors="coerce"
    ).dt.normalize()
    df["ind_promotion"] = pd.to_numeric(
        df["ind_promotion"], errors="coerce"
    )
    df = df[df["order_week"].isin(target_weeks)]

    by_asin_week = (
        df.groupby(["asin", "order_week"], as_index=False)
        .agg(ind_promotion=("ind_promotion", "max"))
    )
    pivot = by_asin_week.pivot(
        index="asin",
        columns="order_week",
        values="ind_promotion",
    ).reindex(columns=target_weeks)

    complete = pivot.notna().all(axis=1)
    all_zero = pivot.eq(0.0).all(axis=1)
    selected = pivot.loc[complete & all_zero].copy()
    selected.columns = [
        "h1_ind_promotion",
        "h2_ind_promotion",
        "h3_ind_promotion",
    ...
    return set(selected["asin"].astype(str)), selected


def evaluate_no_promotion_h123_after_chris_jit(
    data_raw1,
    forecast_df,
    scot_df,
    expected_weeks,
    ...
    """Evaluate the already-produced forecast on H1-H3 non-promotion ASINs.

    The cohort must already be the final Chris-then-JIT intersection.  An ASIN
    enters this diagnostic only when all three target weeks are present in the
    evaluation snapshot and raw ``ind_promotion`` equals zero in H1, H2, and
    H3.  This function never retrains the model and never overwrites model
    inputs; it only slices the completed out-of-sample predictions.
    """
    required_raw = {"asin", "order_week", "ind_promotion"}
    missing_raw = sorted(required_raw - set(data_raw1.columns))
    if missing_raw:
        raise KeyError(
            "Cannot build the H1-H3 no-promotion diagnostic; evaluation "
            "snapshot is missing: " + ", ".join(missing_raw)
        )
    ...
    if missing_forecast:
        raise KeyError(
            "Cannot evaluate H1-H3 no-promotion forecasts; forecast is "
            "missing: " + ", ".join(missing_forecast)
        )
    ...
    raw["order_week"] = pd.to_datetime(
        raw["order_week"], errors="coerce"
    ).dt.normalize()
    raw["ind_promotion"] = pd.to_numeric(
        raw["ind_promotion"], errors="coerce"
    )
    raw = raw[
        raw["order_week"].isin(target_weeks)
    ][["asin", "order_week", "ind_promotion"]].copy()

    # If duplicate ASIN-week rows exist, max is conservative: one promotion
    # record makes that ASIN-week promotional.  Missing values stay missing and
    # therefore cannot be mistaken for zero promotion.
    promo_by_asin_week = (
        raw.groupby(["asin", "order_week"], as_index=False)
        .agg(ind_promotion=("ind_promotion", "max"))
    )
    promo_pivot = promo_by_asin_week.pivot(
        index="asin",
        columns="order_week",
        values="ind_promotion",
    ).reindex(columns=target_weeks)

    complete_h123 = promo_pivot.notna().all(axis=1)
    zero_h123 = promo_pivot.eq(0.0).all(axis=1)
    no_promo_asins = set(
        promo_pivot.index[complete_h123 & zero_h123].astype(str)
    )

    pred = forecast_df.copy()
    pred["asin"] = pred["asin"].astype(str).str.strip()
    pred["order_week"] = pd.to_datetime(
        pred["order_week"], errors="coerce"
    ).dt.normalize()
    predicted_asins = set(pred["asin"].dropna().unique())
    no_promo_asins &= predicted_asins

    if not no_promo_asins:
        raise RuntimeError(
            "Zero ASINs have complete H1/H2/H3 rows with ind_promotion=0 "
            "after the Chris-then-JIT intersection."
        )

    no_promo_forecast = pred[
        pred["asin"].isin(no_promo_asins)
        & pred["order_week"].isin(target_weeks)
    ].copy()
    no_promo_forecast = no_promo_forecast.merge(
        promo_by_asin_week,
        on=["asin", "order_week"],
        how="left",
        validate="many_to_one",
    ...
    observed_counts = no_promo_forecast.groupby("asin")[
        "order_week"
    ].nunique()
    incomplete_prediction_asins = set(
        observed_counts[observed_counts != 3].index.astype(str)
    )
    if incomplete_prediction_asins:
        raise RuntimeError(
            f"{len(incomplete_prediction_asins):,} selected no-promotion "
            "ASINs do not have exactly three prediction weeks."
        )
    if not no_promo_forecast["ind_promotion"].eq(0.0).all():
        raise RuntimeError(
            "Internal assertion failed: filtered H1-H3 cohort contains a "
            "nonzero ind_promotion row."
        )

    output_root = Path(output_root)
    output_root.mkdir(parents=True, exist_ok=True)

    cohort = promo_pivot.loc[sorted(no_promo_asins), target_weeks].copy()
    cohort.columns = [
        "h1_ind_promotion",
        "h2_ind_promotion",
        "h3_ind_promotion",
    ...
    cohort_path = output_root / (
        f"no_promotion_h123_asins_cut_{cut_token}.csv"
    )
    cohort.to_csv(cohort_path, index=False)

    prediction_path = output_root / (
        f"prediction_no_promotion_h123_cut_{cut_token}.csv"
    )
    build_prediction_export_df(no_promo_forecast).to_csv(
        prediction_path,
        index=False,
    )

    print("\n" + "=" * 96)
    print("NO-PROMOTION H1-H3 DIAGNOSTIC | AFTER CHRIS THEN JIT")
    print("=" * 96)
    print("Target weeks:", [str(w.date()) for w in target_weeks])
    print(f"Final joint forecast ASINs: {len(predicted_asins):,}")
    print(f"H1/H2/H3 all-zero-promotion ASINs: {len(no_promo_asins):,}")
    print(
        "No-promotion ASIN share: "
        f"{len(no_promo_asins) / max(len(predicted_asins), 1):.2%}"
    )
    print(f"No-promotion forecast rows: {len(no_promo_forecast):,}")
    print(f"Saved no-promotion ASIN cohort: {cohort_path}")
    print(f"Saved no-promotion predictions: {prediction_path}")

    evaluation = _evaluate_standard_wape_against_scot(
        result={"forecast_df": no_promo_forecast},
        scot_df=scot_df,
        data_raw1=data_raw1,
        remove_oos_dp=remove_oos_dp,
        source="lp",
    ...
    for quantile in ["p50", "p70", "p90"]:
        table = evaluation.get(f"h_wape_{quantile}")
        if isinstance(table, pd.DataFrame) and not table.empty:
            table = table.copy()
            table.insert(0, "quantile", quantile)
            horizon_tables.append(table)

    horizon_summary = (
        pd.concat(horizon_tables, ignore_index=True)
        if horizon_tables else pd.DataFrame()
    )
    wape_path = output_root / (
        f"wape_no_promotion_h123_cut_{cut_token}.csv"
    )
    horizon_summary.to_csv(wape_path, index=False)
    print(f"Saved no-promotion H1/H2/H3 WAPE: {wape_path}")

    comparison_tables = []
    if isinstance(all_evaluation, dict):
        for quantile in ["p50", "p70", "p90"]:
            all_table = all_evaluation.get(f"h_wape_{quantile}")
            no_promo_table = evaluation.get(f"h_wape_{quantile}")
            for cohort_name, table in [
                ("all_chris_then_jit", all_table),
                ("h123_all_ind_promotion_zero", no_promo_table),
            ]:
                if isinstance(table, pd.DataFrame) and not table.empty:
                    table = table.copy()
                    table.insert(0, "cohort", cohort_name)
                    table.insert(1, "quantile", quantile)
                    comparison_tables.append(table)

    comparison = (
        pd.concat(comparison_tables, ignore_index=True)
        if comparison_tables else pd.DataFrame()
    )
    comparison_path = output_root / (
        f"wape_all_vs_no_promotion_h123_cut_{cut_token}.csv"
    )
    comparison.to_csv(comparison_path, index=False)
    print(f"Saved ALL vs no-promotion WAPE comparison: {comparison_path}")

    return {
        "asin_count": len(no_promo_asins),
        "forecast_asin_count": len(predicted_asins),
        "asin_share": len(no_promo_asins) / max(len(predicted_asins), 1),
        "cohort": cohort,
        "forecast_df": no_promo_forecast,
        "horizon_wape": horizon_summary,
        "all_vs_no_promotion_wape": comparison,
        "evaluation": evaluation,
        "cohort_path": str(cohort_path),
        "prediction_path": str(prediction_path),
        "wape_path": str(wape_path),
...
# =====================================================
# JIT ASIN cohort — applied after the Chris intersection
# =====================================================
...
def _resolve_jit_asin_csv(jit_asin_path=None):
    ...
    if not matches:
        raise FileNotFoundError(
            f"No {DEFAULT_JIT_ASIN_GLOB} found. Pass jit_asin_path explicitly."
        )
    raise RuntimeError(
        "Multiple JIT ASIN CSVs found; pass jit_asin_path explicitly:\n  "
...
def _load_jit_asin_set(jit_asin_path=None):
    ...
    if "asin" not in by_lower:
        raise KeyError(f"JIT CSV has no ASIN column: {list(header.columns)}")
    ...
    print(f"[JIT] loaded {len(asin_set):,} ASINs from {path}", flush=True)
...
FEATURE_IMPORTANCE_CONTEXT_COLS = [
    "our_price",
...
def _safe_wape_for_importance(actual, predicted):
...
def _importance_eval_one_permutation(
    model, va_ld, context_col_idx=None, M=100, seed=42,
):
    values = {
        "oos": [], "demand": [], "p50": [], "p70": [], "p90": [],
        "total": [], "total_hat": [], "buybox": [], "buybox_hat": [],
        "instock": [], "instock_hat": [],
    }
    model = model.to(DEVICE)
    model.eval()
    with torch.no_grad():
        for batch_idx, original_batch in enumerate(va_ld):
            ...
            if context_col_idx is not None:
                flat = fc[:, :, context_col_idx].reshape(-1)
                rng = np.random.default_rng(seed + 10007 * (batch_idx + 1))
                perm = torch.as_tensor(
                    rng.permutation(flat.numel()), dtype=torch.long,
                    device=flat.device,
                )
                fc[:, :, context_col_idx] = flat[perm].reshape_as(
                    fc[:, :, context_col_idx]
                )

            # Identical NB Monte Carlo draws for baseline and permutations.
            ...
            values["oos"].append(b["oos"].detach().cpu().numpy().ravel())
            values["demand"].append(b["y"].detach().cpu().numpy().ravel())
            values["p50"].append(p50.detach().cpu().numpy().ravel())
            values["p70"].append(p70.detach().cpu().numpy().ravel())
            values["p90"].append(p90.detach().cpu().numpy().ravel())
            values["total"].append(b["future_total_dph"].detach().cpu().numpy().ravel())
            values["total_hat"].append(exp_hat[:, :, 0].detach().cpu().numpy().ravel())
            values["buybox"].append(b["future_buy_box_dph"].detach().cpu().numpy().ravel())
            values["buybox_hat"].append(exp_hat[:, :, 1].detach().cpu().numpy().ravel())
            values["instock"].append(b["future_instock"].detach().cpu().numpy().ravel())
            values["instock_hat"].append(exp_hat[:, :, 2].detach().cpu().numpy().ravel())

    arrays = {k: np.concatenate(v) for k, v in values.items()}
    keep = np.isfinite(arrays["oos"]) & (arrays["oos"] == 0)
    arrays = {k: v[keep] for k, v in arrays.items() if k != "oos"}
    return {
        "demand_p50_wape": _safe_wape_for_importance(arrays["demand"], arrays["p50"]),
        "demand_p70_wape": _safe_wape_for_importance(arrays["demand"], arrays["p70"]),
        "demand_p90_wape": _safe_wape_for_importance(arrays["demand"], arrays["p90"]),
        "total_dph_wape": _safe_wape_for_importance(arrays["total"], arrays["total_hat"]),
        "buy_box_dph_wape": _safe_wape_for_importance(arrays["buybox"], arrays["buybox_hat"]),
        "in_stock_dph_wape": _safe_wape_for_importance(arrays["instock"], arrays["instock_hat"]),
    }


def compute_four_feature_permutation_importance(
    model, va_ld, context_cols, M=100, seed=42,
):
    missing = [c for c in FEATURE_IMPORTANCE_CONTEXT_COLS if c not in context_cols]
    if missing:
        raise RuntimeError(
            "Requested importance features missing from future_context: "
            + ", ".join(missing)
        )
    baseline = _importance_eval_one_permutation(
        model, va_ld, context_col_idx=None, M=M, seed=seed
    )
    rows = []
    for encoded_col in FEATURE_IMPORTANCE_CONTEXT_COLS:
        permuted = _importance_eval_one_permutation(
            model, va_ld, context_col_idx=context_cols.index(encoded_col),
            M=M, seed=seed,
        )
        feature = encoded_col.replace("stock_extra__", "")
        row = {"feature": feature, "context_column": encoded_col}
        for metric, base_value in baseline.items():
            perm_value = permuted[metric]
            row[f"baseline_{metric}"] = base_value
            row[f"permuted_{metric}"] = perm_value
            row[f"importance_delta_{metric}"] = perm_value - base_value
            row[f"importance_pct_{metric}"] = (
                100.0 * (perm_value - base_value) / base_value
                if np.isfinite(base_value) and base_value > 0 else np.nan
            )
        rows.append(row)
    ...
    show = [
        "feature", "importance_pct_demand_p50_wape",
        "importance_pct_demand_p70_wape", "importance_pct_total_dph_wape",
        "importance_pct_buy_box_dph_wape", "importance_pct_in_stock_dph_wape",
    ]
    print("\n" + "=" * 112, flush=True)
    print("OUR_PRICE + THREE PROMOTION FEATURES: PERMUTATION IMPORTANCE", flush=True)
    print("Positive WAPE increase = useful; larger increase = more important.", flush=True)
    print("=" * 112, flush=True)
    print(result[show].round(4).to_string(index=False), flush=True)
    return result
...
def run_joint_exposure_demand_h3_end2end(
    ...
    if "our_price" not in context_cols:
        raise RuntimeError(
            "our_price was requested as a future feature but is missing from "
            "context_cols."
        )
    print(
        f"[OUR-PRICE] enabled in future_context | "
        f"context_index={context_cols.index('our_price')} | transform=log1p",
        flush=True,
    )
    ...
    if run_feature_importance:
        feature_importance = compute_four_feature_permutation_importance(
            model=model,
            va_ld=va_ld,
        ...
        feature_importance_path = str(
            output_csv.parent / f"four_feature_importance_{output_csv.stem}.csv"
        )
        feature_importance.to_csv(feature_importance_path, index=False)
        print(f"Saved feature importance: {feature_importance_path}", flush=True)
...
def run_joint_h3_s3_rolling_scot_p50_p70(
    ...
    data_prefix=ROLLING_DATA_PREFIX,
    scot_prefix=ROLLING_SCOT_PREFIX,
    joint_jit_after_chris=False,
    jit_asin_path=None,
    run_feature_importance=False,
    evaluate_no_promotion_h123=False,
    restrict_model_to_no_promotion_h123=False,
):
    """
    ...
    if run_feature_importance and resume_existing:
        raise ValueError(
            "Set resume_existing=False when run_feature_importance=True; "
            "permutation importance requires the trained model in memory."
        )

    jit_asin_set = None
    resolved_jit_path = None
    if joint_jit_after_chris:
        jit_asin_set, resolved_jit_path = _load_jit_asin_set(jit_asin_path)
    ...
    print("our_price future feature: ON (log1p)")
    print("Chris then JIT intersection:", "ON" if joint_jit_after_chris else "OFF")
    if resolved_jit_path is not None:
        print("JIT cohort CSV:", resolved_jit_path)
    print("Four-feature permutation importance:", "ON" if run_feature_importance else "OFF")
    print(
        "H1-H3 all-zero ind_promotion evaluation:",
        "ON" if evaluate_no_promotion_h123 else "OFF",
    )
    print(
        "Restrict model cohort to H1-H3 all-zero ind_promotion:",
        "ON" if restrict_model_to_no_promotion_h123 else "OFF",
    )
    print(
    ...
    for run_index, row in pairs.iterrows():
        ...
        try:
            ...
            print(
                ...
                f"joint={len(chris_joint_asins):,}",
                flush=True,
            )

            if joint_jit_after_chris:
                eligible_joint_asins = sorted(chris_joint_asins & jit_asin_set)
                print(
                    f"[JIT-AFTER-CHRIS] chris_joint={len(chris_joint_asins):,} | "
                    f"jit={len(jit_asin_set):,} | final_joint="
                    f"{len(eligible_joint_asins):,}",
                    flush=True,
                )
            else:
                eligible_joint_asins = sorted(chris_joint_asins)
            ...
            # Oracle diagnostic experiment: use the later evaluation snapshot
            # only to choose ASINs that are non-promotional in all three target
            # weeks.  Once selected, the model is trained, predicted, and
            # evaluated only on this restricted final joint cohort.
            if restrict_model_to_no_promotion_h123:
                restriction_weeks = [
                    scot_fcd,
                    scot_fcd + pd.Timedelta(days=7),
                    scot_fcd + pd.Timedelta(days=14),
                ]
                no_promo_set, no_promo_detail = (
                    select_complete_h123_no_promotion_asins(
                        eval_raw=eval_raw,
                        expected_weeks=restriction_weeks,
                    )
                )
                before_no_promo = len(eligible_joint_asins)
                eligible_joint_asins = sorted(
                    set(eligible_joint_asins) & no_promo_set
                )
                no_promo_detail = no_promo_detail[
                    no_promo_detail["asin"].isin(eligible_joint_asins)
                ].sort_values("asin").reset_index(drop=True)

                print(
                    "[ORACLE NO-PROMO H1-H3 MODEL COHORT] "
                    f"before={before_no_promo:,} | "
                    f"after={len(eligible_joint_asins):,} | "
                    "condition=(H1=0 AND H2=0 AND H3=0)",
                    flush=True,
                )
                no_promo_cohort_path = output_root / (
                    "model_cohort_h123_no_promotion_cut_"
                    f"{cut_token}.csv"
                )
                no_promo_detail.to_csv(
                    no_promo_cohort_path,
                    index=False,
                )
                print(
                    "[ORACLE NO-PROMO H1-H3 MODEL COHORT] saved: "
                    f"{no_promo_cohort_path}",
                    flush=True,
                )

                if not eligible_joint_asins:
                    raise RuntimeError(
                        "No ASINs remain after requiring complete H1/H2/H3 "
                        "ind_promotion=0 in the evaluation snapshot."
                    )
            ...
            no_promotion_diag = None
            if evaluate_no_promotion_h123:
                no_promotion_diag = evaluate_no_promotion_h123_after_chris_jit(
                    data_raw1=data_raw1,
                    forecast_df=forecast_df,
                    scot_df=scot_df,
                    expected_weeks=[expected_h1, expected_h2, expected_h3],
                    output_root=output_root,
                    cut_token=cut_token,
                    remove_oos_dp=remove_oos_dp,
                    all_evaluation=real_scot,
                )
            ...
            summary = {
                ...
                "no_promotion_h123_asins": (
                    no_promotion_diag["asin_count"]
                    if no_promotion_diag is not None else np.nan
                ),
                "no_promotion_h123_asin_share": (
                    no_promotion_diag["asin_share"]
                    if no_promotion_diag is not None else np.nan
                ),
...
# ============================================================================
# USAGE 4: TWO FCDs | CHRIS THEN JIT | ONLY H1-H3 NO-PROMOTION ASINs
# ============================================================================
# Cohort construction:
#   origin ∩ eval ∩ SCOT ∩ Chris → intersect JIT CSV
# For each FCD, the final Chris-then-JIT cohort is restricted BEFORE sampling
# and model execution to ASINs whose raw eval ind_promotion is zero in H1, H2,
# and H3.  The model and SCOT WAPE are therefore run only on this one cohort.
# This is an oracle diagnostic because cohort selection uses the later eval.
#
# If multiple matching JIT files exist, set the exact path below.
JIT_ASIN_CSV_PATH = None

two_fcd_chris_then_jit_only_no_promotion_h123 = (
    run_joint_h3_s3_rolling_scot_p50_p70(
        n_asins=None,
        seed=42,
        max_snapshots=2,
        select_latest=False,
        ...
        detach_exposure_for_demand=False,
        output_root=(
            "joint_h3_two_fcd_chris_then_jit_"
            "only_no_promotion_h123"
        ),
        resume_existing=False,
        continue_on_error=False,
        joint_jit_after_chris=True,
        jit_asin_path=JIT_ASIN_CSV_PATH,
        run_feature_importance=False,
        evaluate_no_promotion_h123=False,
        restrict_model_to_no_promotion_h123=True,
    )
)
